...
const checkoutEmailInput = ref(props.checkoutEmail || props.discount?.email || '')
...
const checkoutCustomerEmail = computed(() => checkoutEmailInput.value.trim())
const hasCheckoutEmail = computed(() => EMAIL_PATTERN.test(checkoutCustomerEmail.value))
const hasSavedDiscount = computed(() => Boolean(
  appliedDiscount.value?.code &&
  normalizedCoupon.value === String(appliedDiscount.value.code).toUpperCase() &&
...
function launchOfferConfetti() {
  ...
  const fallingPieces = isMobile ? [] : Array.from({ length: 34 }, (_, index) => ({
    id: `${timestamp}-${index}`,
  ...
  const burstPieces = ['left', 'right'].flatMap((side) => (
    Array.from({ length: isMobile ? 16 : 18 }, (_, index) => {
      ...
      const drift = direction * (isMobile ? 82 + Math.random() * 128 : 120 + Math.random() * 280)
      const lift = isMobile ? -(62 + Math.random() * 128) : -120 + Math.random() * 250
      ...
      return {
        ...
        left: side === 'left' ? (isMobile ? '-12vw' : '4vw') : (isMobile ? '112vw' : '96vw'),
        top: `${isMobile ? 90 + Math.random() * 8 : 34 + Math.random() * 24}vh`,
        drift: `${drift}px`,
        midDrift: `${drift * (isMobile ? 0.42 : 0.72)}px`,
        lift: `${lift}px`,
        rotation: `${rotation}deg`,
        midRotation: `${rotation * 0.58}deg`,
        delay: `${isMobile ? Math.random() * 55 : 80 + Math.random() * 150}ms`,
        duration: `${isMobile ? 680 + Math.random() * 260 : 1250 + Math.random() * 650}ms`,
    ...
    <form v-else class="mello-checkout-body" @submit.prevent="submitCheckout">
      ...
      <aside class="mello-checkout-summary" :aria-label="t('checkout.summary.title')">
        <div class="mello-checkout-summary__inner">
          ...
          <section class="mello-checkout-summary-contact" aria-labelledby="checkout-contact">
            <div class="mello-checkout-contact-line">
...
.mello-checkout-contact-line svg,
...
.mello-checkout-contact-line h2,
.mello-checkout-contact-line h3,
...
.mello-checkout-summary-contact,
...
.mello-checkout-summary-contact__field,
...
.mello-checkout-summary-contact input,
